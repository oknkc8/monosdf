...
def get_sphere_intersection_volSDF(cam_loc, ray_directions, r = 1.0):
    # Input: n_images x 4 x 4 (not n_images x 3) ; n_images x n_rays x 3
    # Output: n_images * n_rays x 2 (close and far) ; n_images * n_rays

    n_imgs, n_pix, _ = ray_directions.shape

    cam_loc = cam_loc.unsqueeze(-1)
    ray_cam_dot = torch.bmm(ray_directions, cam_loc).squeeze()  # n_imgs x n_rays
    under_sqrt = ray_cam_dot ** 2 - (cam_loc.norm(2,1) ** 2 - r ** 2)

    under_sqrt = under_sqrt.reshape(-1)
    mask_intersect = under_sqrt > 0

    sphere_intersections = torch.zeros((n_imgs * n_pix, 2), device=cam_loc.device, dtype=torch.float)
